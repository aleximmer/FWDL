...
def plot_input_activations(weights, aggst, run):
    ...
    plt.imshow(agg(np.abs(weights), axis=0).reshape(28, 28), cmap='gray')
    plt.title('SGD')
    plt.xticks([])
    plt.yticks([])
    plt.tight_layout()
    i = np.random.randint(1000)
    plt.savefig('figure{i}.png'.format(i=str(i)))
    run.add_artifact('figure{i}.png'.format(i=str(i)),
                     'NetActivation-{aggst}.png'.format(aggst=aggst))
    plt.gcf().clear()
...
def plot_sparse_network(weights, run):
    ...
    i = np.random.randint(1000)
    plt.savefig('figure{i}.png'.format(i=str(i)))
    run.add_artifact('figure{i}.png'.format(i=str(i)), 'Sparsity.png')
    plt.gcf().clear()
